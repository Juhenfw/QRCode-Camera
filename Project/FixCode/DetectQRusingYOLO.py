...
while (cv2.waitKey(1) & 0xFF) != 27:
    try:
        ...
        if frame_count % 1 == 0:
            ...
            decoded_data = qr_reader.detect_and_decode(th)
            ...
            if decoded_data and (time.time() - last_qr_print_time) >= qr_print_interval:
                print("QR Code detected: ")
                print(f"{decoded_data}")
                last_qr_print_time = time.time()
                last_no_qr_print_time = time.time()
            elif not decoded_data and (time.time() - last_no_qr_print_time) >= qr_print_interval:
